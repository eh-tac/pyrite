...
export abstract class FlightGroupBase extends PyriteBase implements Byteable {
  ...
  constructor(hex: ArrayBuffer, tie?: IMission) {
    ...
    this.Name = getChar(hex, 0x000, undefined);
    this.Pilot = getChar(hex, 0x00C, undefined);
    this.Cargo = getChar(hex, 0x018, undefined);
    this.SpecialCargo = getChar(hex, 0x024, undefined);
  ...
  public toHexString(): string {
    ...
    writeChar(hex, this.Pilot, 0x00C);
    writeChar(hex, this.Cargo, 0x018);
    writeChar(hex, this.SpecialCargo, 0x024);
    ...
    writeByte(hex, 0, 0x05D);
    writeByte(hex, this.Unknown16, 0x05E);
    writeByte(hex, 0, 0x05F);
    ...
    for (let i = 0; i < 3; i++) {
      ...
      writeObject(hex, t, 0x068);
      offset += t.getLength();
    }
    offset = 0x09E;
    for (let i = 0; i < 4; i++) {
      const t = this.FlightGroupGoals[i];
      writeObject(hex, t, 0x09E);
      offset += t.getLength();
    }
