...
import { IFGScore, IFlightGroup } from "../pyrite-base";
import { FlightGroupBase } from "./base/flight-group-base";
import { ArrivalDifficulty, CraftAbbr } from "./constants";
...
export class FlightGroup extends FlightGroupBase implements IFlightGroup, IFGScore {
  ...
  public get CraftTypeAbbr(): string {
    return Constants.CRAFTABBR[this.CraftType as unknown as CraftAbbr] || `UnknownCraftType${this.CraftType}`;
  }
  ...
  public pointValue(level: "Easy" | "Medium" | "Hard"): number {
    // Implement the logic to return the point value for the specified difficulty level
    return 0;
  }
}
