...
export class TypeScriptWriter extends PyriteWriter {
  public constructor(
    rootDir: string,
    public componentsRootDir: string,
    generator,
  ...
  public write(): this {
    ...
    this.writeComponentFile("controllers/PLT/index.ts", contrIndex.join("\n"));
...
export class ${this.controllerName(struct)} extends ControllerBase {
  ...
  constructor(public model: ${struct.name}){
    ...
    this.writeComponentFile(`controllers/PLT/${this.filename(struct.name)}`, contents);
  }
  ...
  public writeComponent(struct: Struct, props: TypeScriptPropWriter[]): void {
    ...
    this.writeComponentFile(`components/PLT/${kebab}/${scssFile}`, "", false);
...
export class ${compName} {
  ...
  `;
    const file = this.filename(struct.name);
    this.writeComponentFile(`components/PLT/${kebab}/${file}x`, contents, false);
  }
  ...
  protected writeComponentFile(path: string, contents: string, overwriteIfExists: boolean = true): void {
    const currentRoot = this.rootDir;
    this.rootDir = this.componentsRootDir;
    this.writeFile(path, contents, overwriteIfExists);
    this.rootDir = currentRoot;
  }
