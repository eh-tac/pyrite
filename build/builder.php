...
TXT;
	...
	{
		$ns = "\\Pyrite\\" . $this->platform . "\\";
		return "\t/** @var {$ns}{$prop['type']} */" . PHP_EOL .
	...
	{
		$ns = "\\Pyrite\\" . $this->platform . "\\";
		return "new {$ns}$typeName(substr($h, $offset), \$this->TIE)";
	}
...
ELU;
	...
	protected function writeClass(array $lines, $className)
	{
		...
		file_put_contents("{$this->out}{$this->platform}/Base/{$className}Base.php", $this->filterFileContents($fileContents));
		echo "\nWrote stuff to {$this->out}{$this->platform}/Base/{$className}Base.php";
		//		echo htmlentities($fileContents);

		$impl     = "<?php
namespace Pyrite\{$this->platform};
